""" Generate certificate required to leave domicile in France during Covid crisis.
...
Starting on April 6th, a digital version of the certificate (QR code) can be
generated using a web service run by the authorities (with a strong message
that all data collected by the web service remains on the user's terminal and
no information is collected by the authorities in the process).
...
certificate is to be generated. This applet provides a convenient alternative
to the official service:

- Personal details for yourself and all the persons you are confined with need
  only be keyed in once, in file :file:`persons.json`, in the same directory as
  this script. For an example, see :file:`persons_example.json`, in the same
  directory.
...
- In all cases I have tested, the information contained in the PDF file and the
  ...
  cannot guarantee this is true in all cases -- Use at your own risk.
